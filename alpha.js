...
async function avGet(params, avKey) {
    ...
    if (!res.ok)
        throw new Error(`Alpha Vantage ${params.function} HTTP ${res.status}`);
    return res.json();
}
async function fetchOverview(ticker, avKey) {
    try {
        const data = await avGet({ function: 'OVERVIEW', symbol: ticker }, avKey);
        if (!data.Symbol)
            return null;
        return {
            symbol: data.Symbol,
...
async function fetchQuarterlyFinancials(ticker, avKey) {
    try {
        const data = await avGet({ function: 'INCOME_STATEMENT', symbol: ticker }, avKey);
        if (!data.quarterlyReports?.length)
            return [];
...
async function fetchEpsHistory(ticker, avKey) {
    try {
        const data = await avGet({ function: 'EARNINGS', symbol: ticker }, avKey);
        if (!data.quarterlyEarnings?.length)
            return [];
...
async function fetchAvCalendar(avKey, horizon = '3month') {
    try {
...
// ---------------------------------------------------------------------------
async function fetchNasdaqCalendar(date) {
    try {
...
// Scoring engine
// ---------------------------------------------------------------------------
function score(quarters, eps, overview) {
    let total = 0;
    const lines = [];
    ...
    if (quarters.length >= 8) {
        ...
        for (let i = 0; i < 4; i++) {
            ...
            if (prior > 0)
                yoyRates.push(((curr - prior) / prior) * 100);
        }
...
function buildReport(ticker, overview, quarters, eps, sig, aiContext) {
...
async function fetchAiContext(runPrompt, ticker, name) {
    const today = new Date().toLocaleDateString('en-US', {
        month: 'long', day: 'numeric', year: 'numeric',
    });
    const prompt = ALPHA_SYSTEM + '\n\n' +
...
// ---------------------------------------------------------------------------
async function analyzeOne(ticker, runPrompt, avKey, skipAi = false) {
...
// ---------------------------------------------------------------------------
async function buildWeekCalendar(avKey) {
...
// ---------------------------------------------------------------------------
export async function runAlphaDaily(runPrompt) {
...
// ---------------------------------------------------------------------------
export async function handleAlpha(arg, runPrompt) {
    const avKey = process.env.ALPHA_VANTAGE_API_KEY;
